...
def list_available_metrics(client: MonitorManagementClient) -> None:
    ...
    for m in metric_defs:
        # Show metric name, unit, and available aggregation types
...
def query_call_metrics(
    ...
    try:
        result = client.metrics.list(
            ...
            aggregation="Total",      # Sum all calls in each interval
        )
...
def query_latency_metrics(
    ...
    Latency metrics help identify performance issues:
      - LatencyE2E: End-to-end request latency (client to service and back)
      - Latency: Server-side processing latency

    Percentile aggregations (P50, P90, P95, P99) are more useful than
    average latency for understanding tail latency and SLA compliance.
    """
    end_time   = datetime.now(timezone.utc)
    start_time = end_time - timedelta(hours=hours_back)
...
def query_token_metrics(
    ...
    Token metrics are important for:
      - Cost management (tokens = billing units)
      - Quota monitoring (TPM limits per deployment)
      - Usage forecasting
    """
    end_time   = datetime.now(timezone.utc)
    start_time = end_time - timedelta(hours=hours_back)
...
def show_alert_setup_guidance() -> None:
    ...
    Alerting is a key part of monitoring strategy covered in the AI-102 exam.
    """
    print("\n" + "=" * 60)
